...
exports.run = function () {
  var request = require('request'),
    ...
    // get all news talks
    console.log(_.range(300));
    async.forEachOf(_.range(300),function (offset, key, callback)  {
        console.log(offset);
        callback(getTalk(offset));
    });

    function getTalk(offset) {
      console.log("Offset = " + offset);
      request.get(strformat(process.env.API_TED_TALK_ALL, {limit: 10, offset: offset}), function(req, res) {
        if (typeof res !== "undefined" && typeof res.body !== "undefined") {
          ...
          if (result.counts.this >0) {
            ...
            forEach(talks, function(item, index) {
              ...
              console.log("talkObj = " + talkObj.id);
